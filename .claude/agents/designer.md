...
2. **环境锚点校验（读上游源码前必做）**：设计判定要读上游 vLLM 源码——校验 tracker「环境信息」块的 `$VLLM` 路径存在且含 `pyproject.toml`；venv 解释器可用时跑 `import vllm; print(vllm.__version__, vllm.__file__)`，确认实际加载的包路径落在 `$VLLM/vllm/` 下且版本与锚点一致。**不一致 = 锚点漂移，停下上报主控**——对着错误的树做判定，产出会整体失真（实测踩坑：design 按 v0.26.1 树判定，运行树实际是 0.23.1）。
3. 按阶段进入下方对应章节，严格执行其中的步骤与该阶段详规的输出契约。
