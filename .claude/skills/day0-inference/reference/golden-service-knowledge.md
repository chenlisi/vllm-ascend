...
> **定位**：服务层机制与实现知识库（不驱动流程）。两个消费方：**Designer**——Phase 1 判定服务层适配需求的依据，**本文每一章即一类适配场景**；**Developer**——Phase 2 按服务层 design spec 落地时回查机制细节。判定逻辑与 spec 契约见设计侧规范 `.claude/skills/day0-inference/reference/design/golden-designer/golden-service-designer.md`；落地流程见 `.claude/skills/day0-inference/reference/adapt/golden-adapter/golden-service-adapter.md`。
>
> **核心命题：错配多为静默失败——HTTP 200 ≠ 部署正确。** parser 名合法即可加载，错的是协议语义；只有矩阵化验证能暴露（见「服务矩阵验证」）。
>
> ⚠️ **行号防腐**：引用代码以函数名/类名/注册项名为锚点，不写 `文件:行号`，失效时按符号名 grep 重新定位。

服务层（API server / parser / tokenizer）决定**请求如何被编码与解析**，分两面：**管理面**（parser 往哪挂、三件套怎么配、chat template 形态、effort 怎么管）、**数据面**（请求链流向、双模 parser 写法、失败模式与验收矩阵）。

判定纪律：三件套「同名同代」是语义判定（`kimi_k2` 对 K3 合法但全错），「parser 名能加载」只证明名字合法，不证明协议语义正确。
...
| 流程阶段 | 本文对应内容 | 落点 |
|---|---|---|
| Phase 0 服务层初判 | 三件套候选名同名同代比对（第 2 节）、chat template 形态判定（第 3 节）、effort 档位与 checkpoint 代次对应关系（第 4 节） | 落盘 `./.day0/<model>/preflight/` |
| Phase 1 场景判定 | 本文每一章即一类适配场景，逐章判定 | 服务层 design spec（`./.day0/<model>/design/service-design-spec.md`） |
| Phase 2 落地实现 | 按 spec 方案块回查对应章节的机制细节 | 实现产物落 `./.day0/<model>/impl/` |
| Stage 4 服务矩阵验收 | 第 10 节的 4 项验收矩阵 | 产出落 `./.day0/<model>/acceptance/`，报告不齐备不得签收 |
