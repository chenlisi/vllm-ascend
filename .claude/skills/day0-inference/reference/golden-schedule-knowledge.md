...
> **定位**：调度层机制与实现知识库（不驱动流程）。两个消费方：**Designer**——Phase 1 判定调度层适配需求的依据，**本文每一章即一类适配场景**；**Developer**——Phase 2 按调度层 design spec 落地时回查机制细节。判定逻辑与 spec 契约见设计侧规范 `.claude/skills/day0-inference/reference/design/golden-designer/golden-schedule-designer.md`；落地流程见 `.claude/skills/day0-inference/reference/adapt/golden-adapter/golden-schedule-adapter.md`。
>
> **核心命题：KVCacheSpec 定稿前禁止进入任何性能工作。** block size、分组、prefix 命中语义全部由 spec 导出，事后更改 spec 会级联推翻调度与图模式配置（capture size、verify 步对齐、page 对齐断言全部要重来）。
>
> ⚠️ **行号防腐**：引用代码以函数名/类名/注册项名为锚点，不写 `文件:行号`，行号随版本漂移，失效时按符号名 grep 重新定位；不确定的行号不写。
>
...
| E3 | Attention backend 选择 | 用了 MLA / 稀疏 / 新注意力 | `NPUPlatform.get_attn_backend_cls()` 的 `(use_mla, use_sparse, use_compress)` 分发表 | → `golden-worker-knowledge.md` 数据面 |
...
| E7 | 图模式 | 模型有跨层状态传递、动态控制流 | `check_and_update_config` 里的 cudagraph_mode 收敛；必要时 `enforce_eager` 或加 splitting_ops。跨层状态直接阻塞 ACL Graph 全图捕获，需将状态传递点加入 `splitting_ops` | → `golden-worker-knowledge.md` 数据面 |
| E8 | MoE 通信方式 | MoE 模型 | `ascend_forward_context.py::select_moe_comm_method`（按 SoC + 专家数 + EP size 分发） | → `golden-worker-knowledge.md` 数据面 |
| E9 | 并行约束校验 | TP/EP/DP/PP 有特殊要求 | `check_and_update_config` 加 assert，早期失败优于运行时崩 | → `golden-worker-knowledge.md` 控制面 |
| E10 | 多模态处理器 | 多模态模型 | 类似 `patch/hunyuan_vl_processor_compat.py` 的 processor 兼容 patch（该文件在 `patch/` 根目录） | → `golden-service-knowledge.md` |
| E11 | Worker 类 | 需要定制 worker 行为 | `parallel_config.worker_cls`（默认 `NPUWorker`）。加载期权重映射检查（厂商权重名 vs vLLM 参数名的 missing/unexpected 审计）同属 Worker 控制面 | → `golden-worker-knowledge.md` 控制面 |
...
| 流程阶段 | 本文对应内容 | 落点 |
|---|---|---|
| Phase 1 场景判定 | 本文每一章即一类适配场景，逐章判定（KVCacheSpec 选型与 page size §一.1-3、调度器装配 §一.4、EngineCore E1-E12 §一.5、prefix cache 运行时语义 §二.6、投机解码调度 §二.7、PD 分离与混合注意力 §二.8） | 调度层 design spec（`./.day0/<model>/design/schedule-design-spec.md`）——KVCacheSpec 选型是必备产出，**定稿前禁止进入性能工作**；E1-E12 归属其他层的项按 §一.5 指针分发 |
| Phase 2 落地实现 | 按 spec 方案块回查对应章节的机制细节 | 实现产物落 `./.day0/<model>/impl/` |
| G4 发布门禁 | prefix cache / 投机解码 / PD 的组合回归高发项（§二.6-8）：E2E 回归配置 `tests/e2e/models/configs/<Model>.yaml`，组合矩阵（量化 × 图 × 投机 × CP/PD）按 Designer 清单显式纳入——历史已知问题几乎全部位于叠加组合而非基线 | 组合矩阵回归清单汇入 G4 |

术语约定与文档族一致：**P0/P1/P2 只用于模型级路径判定**（Phase 0 产出），module 级只用**类型 0-5**（Worker 层判定），本层场景以知识库章节为单位，两套编号均不出现。
