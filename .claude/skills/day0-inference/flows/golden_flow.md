...
- **流程跟踪单**：`./.day0/<model>/tracker.md`（主控立项时按 `.claude/skills/day0-inference/reference/tracker_template.md` 实例化；本 flow 启动前确认「当前阶段 = Stage 1」，各子代理启动时先读它）
...
| 角色 | 子代理文件 | 职责 |
|---|---|---|
| Designer | `designer` | 阶段路由器：按 prompt 中的当前阶段加载对应设计规范（`reference/<stage>-designer/`）执行，输出设计文档 |
...
0. **采集原始证据（脚本化，最先做）**：
   ```bash
   mkdir -p $ASCENDBOT_FILE_PATH/preflight
   .venv/bin/python .claude/skills/day0-inference/scripts/preflight_scan.py <模型路径> $ASCENDBOT_FILE_PATH/preflight/raw_evidence.md
   ```
   脚本从 vllm-ascend 仓根运行，采集（**只产信号、不做判定**）：§1 环境定位（vLLM 源码路径 / 上游基线 commit / npu-smi / torch_npu 版本）、§2 config.json 关键信号与全量、§3 上游注册表 grep 原始结果、§4 量化格式、§5 权重名前缀、§6 modeling 类清单、§7 chat template 形态、§8 上游 parser 注册表可用项、§9 OOT 注册表粗比对、§10 torch_npu 符号探测、§11 魔法数字粗扫。**以下步骤全部基于 raw_evidence.md 做判定**；证据缺失项显式标「待环境实测」，禁止编造。
1. **依赖结论表**（读证据文件做判定）：
   - 上游支持状态三态（证据 §3）：registry grep 命中 → 「已合入」；未命中 → 用 `gh search prs --repo vllm-project/vllm <model>` 或 WebSearch 查上游 PR/分支状态：有未合入 PR → 「pre-release 分支」（附 PR 号）；全无 → 「需自持」。**中间态的成熟度判断是你的裁决，不是脚本结论**。
...
2. **五维同构度扫描**（读证据 §2/§5/§6 做判定，每维产出 delta=0 / delta≠0 结论 + 证据引用）：
   - 注意力类型：从证据 §2 的 config 字段推导 `(use_mla, use_sparse, use_compress)` 特征组合（如含 `index_topk` 即 use_sparse=True），对照 `get_attn_backend_cls` 既有查表键（机制见 ascend-oot.md §3，分派表见 `.claude/skills/day0-inference/reference/adapt/golden-adapter/golden-worker-adapter.md` §2.2）；
...
3. **服务层初判**（方法论见 `.claude/skills/day0-inference/reference/adapt/golden-adapter/golden-service-adapter.md`）：parser 三件套（`--tokenizer-mode` / `--tool-call-parser` / `--reasoning-parser`）候选名是否同名同代成套——证据 §8 是上游注册表可用项清单，候选名逐个比对；**「同名同代」是语义判定**（如 `kimi_k2` 对 K3 合法但全错），由你裁决；chat template 形态读证据 §7（有无 Jinja 模板 vs 仅程序化 prompt 编码——后者要求内置 tokenizer-mode，从可选变为强制）；`reasoning_effort` 档位与 checkpoint 代次的对应关系。
4. 三份产物落盘到 `./.day0/<model>/preflight/`（与脚本产出的 `raw_evidence.md` 同目录——结论表每条结论须引用证据章节号，可追溯），字段模板固定：
   - **依赖结论表**：表头（vLLM 基线 commit / 硬件代次 / torch_npu 版本）+ 逐项「检查项 | 执行命令 | 结论 | 证据摘录」；
   - **五维扫描报告**：每维一行「维度 | 信号来源 | delta=0/≠0 | 机制出口 | 证据」；
   - **路径判定与排期**：路径等级 + 判定依据（哪几维有 delta）+ 排期模板套用结果。
5. **G0 路径门禁**：
...
1. 用 Task 工具调用 designer 子代理，**prompt 必须携带当前阶段信息**：
   ...
   输入：Phase 0 三份产物（$ASCENDBOT_FILE_PATH/preflight/）；设计规范按你的阶段路由加载。
   """)
   ```
   不支持命名子代理的环境，把 `.claude/agents/designer.md` 全文注入子代理首条消息，前缀同样的阶段信息。
2. 收集设计文档 → 存 `./.day0/<model>/design/`。**设计完整性检查**：核对设计文档覆盖 `.claude/skills/day0-inference/reference/design/golden-designer/golden-designer.md`「输出设计文档」的全部 12 项章节——重点抽查：判定表**不含 P 级标注**、含 **Golden 基线说明**（G3 精度门禁依赖它）、调度层设计标注「KVCacheSpec 定稿前禁止进入性能工作」。缺项 → 打回 Designer 补。

### Phase 2 — Developer 实现 + UT
1. 用 Task 工具调用 developer 子代理，**prompt 必须携带当前阶段信息**：
   ```
   Task(subagent_type="developer", prompt="""
   当前的阶段是：Stage 1 Golden 基线（跑起来）
   输出根目录：$ASCENDBOT_FILE_PATH
   请按设计文档完成代码适配与 UT。设计文档路径：$ASCENDBOT_FILE_PATH/design/<设计文档>
   """)
   ```
...
1. 用 Task 工具调用 tester 子代理继续真实权重阶段（**prompt 必须携带当前阶段信息**：「当前的阶段是：Stage 1 Golden 基线，当前 Phase 4（真实权重）」；上下文不足时新开会话，prompt 中附 tester 角色文件路径与 Phase 3 产物路径），**输入 += Designer 的 Golden 基线说明**：去掉 `--load-format dummy` 重新拉起，加载日志 grep `not initialized|size mismatch|shape mismatch`（匹配文案随 vLLM 版本变化——当前版本实测缺失输出为 "Following weights were not initialized from"，**以当前安装版本实测校准**；`Unexpected extra config keys` 属配置项校验，与权重缺失无关，不作阻断项）。
...
3. **G4 发布门禁**：
   - E2E 回归配置生成：`tests/e2e/models/configs/<Model>.yaml`——**格式抄同目录既有配置**（如 `Llama-3.2-3B-Instruct.yaml`），组合矩阵（量化 × 图 × 投机 × CP/PD）按 Designer 清单显式纳入——历史已知问题几乎全部位于叠加组合而非基线；
   ...
   - 教程与支持矩阵：生成 `docs/source/tutorials/models/<Model>.md` 教程（格式参考同目录既有教程）并更新支持矩阵 `docs/source/user_guide/support_matrix/supported_models.md`（与官方 model-adapter skill 的交付标准对齐）；
